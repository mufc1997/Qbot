...
WMRGBLed::WMRGBLed(uint8_t port)//根据接口号寻找相应GPIO
{
...
WMRGBLed::WMRGBLed(uint8_t pin1,uint8_t led_num)
{
	pin = pin1;		
	pinMask = digitalPinToBitMask(pin);
  	ws2812_port   = portOutputRegister(digitalPinToPort(pin));
...
/**
 * Destructor which can call your own function
 */
WMRGBLed::~WMRGBLed(void)
{
  free(pixels);
  pixels = NULL;
}

void WMRGBLed::reset(uint8_t port)
{
  pin = wmPort[port].pin1;
  pinMask = digitalPinToBitMask(pin);
  ws2812_port = portOutputRegister(digitalPinToPort(pin) );
...
void WMRGBLed::setNumber(uint8_t num_leds)
{
  count_led = num_leds;
...
RGB WMRGBLed::getColorAt(uint8_t index)
{
  RGB px_value;
...
uint8_t WMRGBLed::getNumber(void)
{
  return(count_led);
}


bool WMRGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index < count_led)
  {
    uint8_t tmp = index * 3;
...
bool WMRGBLed::setColor(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index == 0)
  {
...
bool WMRGBLed::setColor(uint8_t red, uint8_t green, uint8_t blue)
{
  return(setColor(0, red, green, blue) );;
}


bool WMRGBLed::setColor(uint8_t index, long value)
{
	uint8_t red; uint8_t green; uint8_t blue;
	red = (uint8_t)(value >> 16);
	green =(uint8_t)(value >>8 & 0x00ff);
 ...
 *   None
 */
void WMRGBLed::rgbled_sendarray_mask(uint8_t *data, uint16_t datlen, uint8_t maskhi, uint8_t *port)
{
...
void WMRGBLed::setpin(uint8_t port)
{
